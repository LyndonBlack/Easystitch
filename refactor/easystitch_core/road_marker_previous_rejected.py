...
def _sample_centerline_nodes(cl: LineString, spacing: float = 80.0,
                             bend_angle_deg: float = 45.0) -> list[tuple]:
    """
    Sample a centerline polyline into node positions.
    Places nodes at regular ~`spacing` intervals PLUS at any tight bend
    where the direction changes by more than `bend_angle_deg`.
    Returns list of (x, y, dist_along_cl).
    """
    import math
    total = cl.length
    if total < 1.0:
    ...
    n_samples = max(2, int(total / spacing) + 2)
    sample_dists = [i * total / (n_samples - 1) for i in range(n_samples)]

    # Add tight bend points
    coords = list(cl.coords)
    for i in range(1, len(coords) - 1):
        x0, y0 = coords[i - 1]
        x1, y1 = coords[i]
        x2, y2 = coords[i + 1]
        # Angle between vectors (x0,y0)→(x1,y1) and (x1,y1)→(x2,y2)
        v1x, v1y = x1 - x0, y1 - y0
        v2x, v2y = x2 - x1, y2 - y1
        ...
        if angle > bend_angle_deg:
            proj = cl.project(Point(x1, y1))
            sample_dists.append(proj)

    # Sort, dedup, and convert to points
    sample_dists.sort()
    points = []
    for d in sample_dists:
        if points and d - points[-1][2] < 1.0:
            continue  # skip near-duplicate
        pt = cl.interpolate(d)
        points.append((float(pt.x), float(pt.y), d))
...
def build_initial_graph(polygons: dict[str, Polygon]) -> RoadMarkedPath:
    """
    Stage 0A graph builder — centreline-based.

    For every satin path polygon:
    1. Compute its centreline using the satin engine's rail/rung midpoint method
    2. Sample the centreline at ~80 px intervals + tight bends → nodes
    3. Find junctions where different paths' centreline nodes are close
    4. Build edges between consecutive nodes on each centreline
    """
    from .geometry import compute_centerline
    import math

    # ── 1. Compute centreline polylines ────────────────────────────────
    ...
    for pid, poly in polygons.items():
        ...
        if len(cl_pts) >= 2:
            paths[pid] = LineString(cl_pts)

    if not paths:
        return RoadMarkedPath(path_id="", rungs={}, nodes={}, edges={},
                              stitch_order=[], boundary_coords=[])

    # ── 2. Sample nodes on each centreline ─────────────────────────────
    all_nodes: dict = {}
    all_edges: dict = {}
    stitch_order: list = []
    node_idx = 0
    edge_idx = 0
    # Track which nodes belong to which path for junction detection
    path_node_positions: dict[str, list[tuple[int, float, float]]] = {}  # path_id -> [(node_idx, x, y)]

    for pid, cl in paths.items():
        samples = _sample_centerline_nodes(cl)
        if len(samples) < 2:
            continue

        pnodes = []  # node indices for this path
        for x, y, _ in samples:
            nid = f"n{node_idx}"
            node_idx += 1
            all_nodes[nid] = Node(id=nid, type="path_node", position=(x, y))
            pnodes.append((int(nid[1:]), x, y))

        path_node_positions[pid] = pnodes

        # Build edges between consecutive nodes
        for k in range(len(pnodes) - 1):
            eid = f"e{edge_idx}"
            edge_idx += 1
            nid_a = f"n{pnodes[k][0]}"
            nid_b = f"n{pnodes[k+1][0]}"
            all_edges[eid] = Edge(id=eid, priority=0,
                                  start_node_id=nid_a, end_node_id=nid_b)
            stitch_order.append(eid)

    # ── 3. Find junctions: nodes from different paths that are close ────
    junction_radius = 35.0  # pixels — nodes closer than this are a junction
    pids = sorted(path_node_positions.keys())

    from collections import defaultdict
    # Group nearby nodes from different paths
    node_groups: list[list[int]] = []  # each group is a list of node_idx integers
    node_to_group: dict[int, int] = {}  # node_idx -> group_idx

    for i in range(len(pids)):
        for j in range(i + 1, len(pids)):
            for ni, x1, y1 in path_node_positions[pids[i]]:
                for nj, x2, y2 in path_node_positions[pids[j]]:
                    d = math.hypot(x1 - x2, y1 - y2)
                    if d < junction_radius:
                        # Merge these two nodes into a junction
                        gi = node_to_group.get(ni, -1)
                        gj = node_to_group.get(nj, -1)
                        if gi >= 0 and gj >= 0 and gi != gj:
                            # Merge groups
                            node_groups[gi].extend(node_groups[gj])
                            for nidx in node_groups[gj]:
                                node_to_group[nidx] = gi
                            node_groups[gj] = []
                        elif gi >= 0:
                            node_groups[gi].append(nj)
                            node_to_group[nj] = gi
                        elif gj >= 0:
                            node_groups[gj].append(ni)
                            node_to_group[ni] = gj
                        else:
                            gidx = len(node_groups)
                            node_groups.append([ni, nj])
                            node_to_group[ni] = gidx
                            node_to_group[nj] = gidx

    # ── 4. Replace junction group nodes with single junction node ───────
    for group in node_groups:
        if len(group) < 2:
            continue   # single-node group, skip
        # Average position
        ...
        for nidx in group:
            ...
            if nid in all_nodes:
                valid.append(nidx)
                total_x += all_nodes[nid].position[0]
                total_y += all_nodes[nid].position[1]
        if len(valid) < 2:
            continue
        avg_x = total_x / len(valid)
        avg_y = total_y / len(valid)

        # Create ONE junction node
        jid = f"n{node_idx}"
        node_idx += 1
        all_nodes[jid] = Node(id=jid, type="junction", position=(avg_x, avg_y))

        # Update all edges that referenced the old nodes to point to the junction
        old_ids = {f"n{n}" for n in valid}
        survivor_idx = min(valid)
        survivor_id = f"n{survivor_idx}"

        # Update edges
        for eid, edge in all_edges.items():
            if edge.start_node_id in old_ids:
                edge.start_node_id = jid
            if edge.end_node_id in old_ids:
                edge.end_node_id = jid

        # Remove old individual nodes
        for n in valid:
            nid = f"n{n}"
            if nid in all_nodes and nid != jid:
                del all_nodes[nid]
